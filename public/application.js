...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://pure-forest-62868.herokuapp.com/',
...
var signOutError = function signOutError(signOutResponse) {
...
// create attackers
var attackerColumns = 6;
var attackerRows = 4;
var attackerHeight = playerHeight / 1.5;
var attackerWidth = playerWidth * 1.3;
var attackerVerticalPadding = 5;
var attacker = new Image();
attacker.src = 'public/img/coral.png';
var attackerX = void 0;
var attackerY = void 0;

var attackerArr = [];
for (var i = 0; i < attackerColumns; i++) {
  attackerArr[i] = [];
  for (var j = 0; j < attackerRows; j++) {
    attackerArr[i][j] = {
      x: 0,
      y: 0,
      status: 1
...
var assignXAndY = function assignXAndY() {
  for (var _i = 0; _i < attackerColumns; _i++) {
    for (var _j = 0; _j < attackerRows; _j++) {
      attackerX = Math.random() * canvas.width + 1;
      attackerY = _j * (attackerHeight + attackerVerticalPadding) + attackerVerticalPadding;
      attackerArr[_i][_j].x = attackerX;
      attackerArr[_i][_j].y = attackerY;
    }
  }
};
...
function drawBullet() {
  if (bullets.length) {
    for (var _i2 = 0; _i2 < bullets.length; _i2++) {
      ctx.fillStyle = '#000';
      ctx.fill();
      ctx.fillRect(bullets[_i2][0], bullets[_i2][1], bullets[_i2][2], bullets[_i2][3]);
    }
  }
}
...
function drawAttackers() {
  for (var _i3 = 0; _i3 < attackerColumns; _i3++) {
    for (var _j2 = 0; _j2 < attackerRows; _j2++) {
      if (attackerArr[_i3][_j2].status === 1) {
        ctx.beginPath();
        ctx.drawImage(attacker, attackerArr[_i3][_j2].x, attackerArr[_i3][_j2].y, attackerWidth, attackerHeight);
        ctx.closePath();
      }
    }
  }
...
function moveAttackers() {
  for (var _i4 = 0; _i4 < attackerArr.length; _i4++) {
    var currentAttacker = attackerArr[_i4];
    for (var _j3 = 0; _j3 < currentAttacker.length; _j3++) {
      if (attackerArr[_i4][_j3].y < canvas.height + 10) {
        attackerArr[_i4][_j3].y += 1;
      } else if (attackerArr[_i4][_j3].y > canvas.height - 1) {
        attackerArr[_i4][_j3].x = Math.random() * canvas.width + 1;
        attackerArr[_i4][_j3].y = -80;
      }
    }
  }
}
...
function moveBullet() {
  for (var _i5 = 0; _i5 < bullets.length; _i5++) {
    if (bullets[_i5][1] > -11) {
      bullets[_i5][1] -= 5;
    } else if (bullets[_i5][1] < -10) {
      bullets.splice(_i5, 1);
    }
  }
}
...
function hitAttacker() {
  ...
  for (var k = 0; k < bullets.length; k++) {
    ...
    for (var _i6 = 0; _i6 < attackerColumns; _i6++) {
      for (var _j4 = 0; _j4 < attackerRows; _j4++) {
        var _attacker = attackerArr[_i6][_j4];
        if (_attacker.status === 1) {
          // allow for margin of error around center of attacker + half of width on either side
          if (_attacker.x + attackerWidth / 2 + 6 >= bullets[k][0] && _attacker.x - attackerWidth / 2 - 4 <= bullets[k][0] && _attacker.y + attackerHeight / 2 >= bullets[k][1] && _attacker.y - attackerHeight / 2 - 4 <= bullets[k][1]) {
            remove = true;
            store.score++;
            _attacker.status = 0;
...
function hitPlayer() {
  for (var _i7 = 0; _i7 < attackerColumns; _i7++) {
    for (var _j5 = 0; _j5 < attackerRows; _j5++) {
      var _attacker2 = attackerArr[_i7][_j5];
      // check if attacker is `active`
      if (_attacker2.status === 1) {
        // match x and y coordinates
        if (_attacker2.y >= 127 && _attacker2.y <= 130 && _attacker2.x + attackerWidth / 2 >= playerX - playerWidth / 2 && _attacker2.x - attackerWidth / 2 <= playerX + playerWidth / 2) {
          store.over = true;
        }
      }
    }
...
function checkForGameOver(e) {
  if (store.won === true) {
    ...
    for (var _i8 = 0; _i8 < attackerColumns; _i8++) {
      for (var _j6 = 0; _j6 < attackerRows; _j6++) {
        var _attacker3 = attackerArr[_i8][_j6];
        _attacker3.status = 0;
      }
    }
    currentGameId += 1;
...
function startGame() {
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  for (var _i9 = 0; _i9 < attackerColumns; _i9++) {
    for (var _j7 = 0; _j7 < attackerRows; _j7++) {
      var _attacker4 = attackerArr[_i9][_j7];
      _attacker4.status = 1;
    }
  }
  currentGameId += 1;
...
var authUi = __webpack_require__(3);
var gameEvents = __webpack_require__(5);
var game = __webpack_require__(4);
...
var onSignUp = function onSignUp(event) {
  ...
  authApi.signUp(data).then(function () {
    return authApi.signIn(data);
  }).then(authUi.signUpSuccess).catch(authUi.signUpError);
};

var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  authApi.signIn(data).then(authUi.signInSuccess).then(authUi.signedInState).catch(authUi.signInError);
};
...
var onSignOut = function onSignOut(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  authApi.signOut(data).then(authUi.signOutSuccess).catch(authUi.signOutFail);
};
...
var getGamesFail = function getGamesFail() {
...
module.exports = __webpack_require__.p + "10ffbd6494744d9685b623dd8079f8b6.png";
